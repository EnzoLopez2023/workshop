...
- **Dashboard contexts:** Projects, Shaper Hub, and Bambu Hub share `/` and a persisted segmented switcher, but keep independent search, counts, libraries, and state panels. Project status filters never appear in either hub. The Projects context places the focus project first, then the searchable/sortable library, live template clone/delete controls, external build-inspiration links, and the optional Shopkeep companion link. Bambu Hub uses image-forward cards with provider, local-image, and local-file evidence. Templates and inspiration do not have standalone routes.
...
- **Forms:** Plan, Shaper, or Bambu import comes first, essentials precede advanced lists, and native fieldsets group the relevant source, media, files, parts, materials, instructions, and save actions. Bambu analysis exposes the provider manifest and access warnings before a potentially long server-side import; protected MakerWorld originals use authenticated Add files by default. When the personal browser bridge is installed in Chrome or Safari, the saved-file section adds one explicit automatic-import action and reports browser authorization, server download, deduplication, expiry, and failure as text. Project editing gains a sticky save bar after the top actions scroll away. Existing project uploads expose per-file progress, completion, failure, and dismissal; new Shaper uploads queue until the project exists and report partial failures instead of implying an atomic save.
