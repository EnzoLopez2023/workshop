...
- **Vite build-time env var pipeline** (`Dockerfile`, `docker-compose.yml`, `deploy.yml`). `VITE_AZURE_CLIENT_ID`, `VITE_AZURE_HOME_TENANT_ID`, `VITE_AZURE_AUTHORITY_TENANT_ID`, `VITE_TABLOOM_API_BASE_URL`, and `VITE_TABLOOM_CLIENT_ID` are baked into the bundle. The home tenant never changes; the authority changes to `common` only after the portal audience is multitenant.
